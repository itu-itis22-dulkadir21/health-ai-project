...
OUTPUT_FOLDER = "test-output_rotated10"
...
def find_best_angle(gray_resized, coarse_step=10, fine_range=1, min_angle=-50, max_angle=50, min_slices=10, max_slices=30):
    def evaluate_angles(angles):
        ...
        for angle in angles:
            ...
            for num_slices in range(15, 26):  # daha dar aralık
                step = w // num_slices
                for i in range(1, num_slices):
                    x = i * step
                    if x < w * 0.1 or x > w * 0.9:
                        continue
